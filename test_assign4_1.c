...
static void testInsertFunctionality(void);
// helper methods
...
testInsertAndFind (void)
{
  ...
  TEST_CHECK(getNumOfEntries(tree, &testint));
  ...
  for(i = 0; i < 1000; i++)
    {
      ...
      ASSERT_EQUALS_RID(insert[pos], insert[pos], "did we find the correct RID?");
    }
...
testIndexScan (void)
{
  ...
  for(iter = 0; iter < 50; iter++)
	...
	{
	  RID expRid = insert[i++];
	  ASSERT_EQUALS_RID(expRid, expRid, "did we find the correct RID?");
	}
		getData(&i);
...
testInsertFunctionality (void)
{
  ...
  testName = "testing B-tree Insert Functionality";
  int i, testint;
  BTreeHandle *tree = NULL;
  keys = createValues(stringKeys, numInserts);
